...
namespace nsIrNec {
  ...
  void EXT_INTx_ISR() {
    ...
    if ((ms - frameStartAtMs) >= 150) {
      state = state_t::pendingFrame; // force clean up of any failed frame
    }
    ...
    if (state == state_t::pendingFrame) {
      ...
      if (isFalling && ((ms - frameStartAtMs) >= 7) && ((ms - frameStartAtMs) <= 11)) {
        // spec = 9mS
        state = state_t::inHeaderSpace;
        inHeaderSpaceAtUs = us;
      }
      else {
        state = state_t::pendingFrame; // error so quit
      }
    } else if (state == state_t::inHeaderSpace) {
      if (isRising && ((us - inHeaderSpaceAtUs) >= 3500) && ((us - inHeaderSpaceAtUs) <= 5500)) {
        // spec = 4.5mS
        state = state_t::inBitMark;
      } else if (isRising && ((us - inHeaderSpaceAtUs) >= 1750) && ((us - inHeaderSpaceAtUs) <= 3250) && ((ms - frameStartAtMs_last) >= 90) && ((ms - frameStartAtMs_last) <= 130)) {
        // spec = 2.5mS / 110mS
      ...
      if (isRising) {
        state = state_t::inBitMark;
        if (((us - bitSpaceStartAtUs) > 1400) && ((us - bitSpaceStartAtUs) < 1900)) {
          //  0 = 562uS ; 1 = 1688 uS
          // 1 found
          bitSet(dataRaw, bitcount);
        } else if (((us - bitSpaceStartAtUs) > 400) && ((us - bitSpaceStartAtUs) < 750)) {
          //  0 = 562uS ; 1 = 1688 uS
          // 0 found - do nothing
        } else {
          state = state_t::pendingFrame; // error so quit
  ...
  void begin() {
    ...
    attachInterrupt(0, EXT_INTx_ISR, CHANGE);
  }
  #else
  void begin(uint8_t intPin) {
    ...
    attachInterrupt(digitalPinToInterrupt(ir_pin) , EXT_INTx_ISR, CHANGE);
  }
  #endif  
